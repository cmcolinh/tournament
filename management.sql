...
BEGIN
 ...
 SELECT g.gamename from tblgame g JOIN tblattributevalue av ON g._key = av._fk_game WHERE av.enddate IS NULL and av._fk_competition = competitionNumber AND av.varname = 'gameInCompetition' AND av.attrvalue = 'Y';
...
CREATE TRIGGER forNewPlayers AFTER INSERT ON tblplayer
 ...
 VALUES (NEW._key, (SELECT _key FROM tblattributename WHERE varname = 'playerName' AND playernotnull = 1 AND gamenotnull = 0 AND competitionnotnull = 0), NEW.name, CONVERT_TZ(NOW(), @@session.time_zone, '+00:00'), NULL, connection_id());
...
CREATE TRIGGER forNewGames AFTER INSERT ON tblgame
 ...
 VALUES (NEW._key, (SELECT _key FROM tblattributename WHERE varname = 'gameName' AND playernotnull = 0 AND gamenotnull = 1 AND competitionnotnull = 0), NEW.gamename, CONVERT_TZ(NOW(), @@session.time_zone, '+00:00'), NULL, connection_id());
 DELETE FROM tempattributevalue WHERE session_id = connection_id();
 END;//
 ...
 FOR EACH ROW
 BEGIN
 --testing that nullness matches the specs for the tblattributename
 ...
 IF (SELECT NOT EXISTS (SELECT * FROM tblattributevalue WHERE _fk_player <=> NEW._fk_player AND _fk_game <=> NEW._fk_game AND _fk_competition <=> NEW._fk_competition AND _fk_attributename = NEW._fk_attributename AND attrvalue = NEW.attrvalue AND enddate IS NULL) = 1) THEN
 ...
 BEGIN
	 IF (SELECT NOT EXISTS (SELECT _fk_player, _fk_game, _fk_competition, attrname, attrvalue, startdate, enddate
	     FROM tempattributevalue 
	     WHERE _fk_player <=> NEW._fk_player 
	     AND _fk_game <=> NEW._fk_game
	     AND _fk_competition <=> NEW._fk_competition
	     AND _attrname <=> NEW.attrname
